- **`6.12`** Изменен url получения данных с сервера SERVER_DOWNLOAD_URL.

- **`6.11`** Более точно переименован хендлер: `onControlPinFirstEnterPress()`.

- **`6.10`** Проведен рефакторинг кода (`Д1`).

- **`6.09`** Проведен рефакторинг кода (улучшена читаемость, сокращен).

- **`6.08`** Изменена фаза драга управляющего пина.

- **`6.07`** Проведен рефакторинг кода (сокращен).

- **`6.06`** Сокращено лишнее копирование(присвоение) элементов (`Д20`).

- **`6.05`** Default тип недвижимости в decodePropertyType — константа (`Д20`).

- **`6.04`** В show-pins и show-card шаблоны сделаны константами (`Д20`).

- **`6.03`** Количество отрисовываемых пинов выражено константой (`Д19`).

- **`6.02`** Применен тернарный оператор в validation, стр.55 (по замечанию `Д17`).

- **`6.01`** Упрощена проверка validation.js стр.55 (по замечанию `Д16`).

- **`6.00`** Исправлены неверные координаты пинов (по замечанию `Б1`).

- **`5.21`** Реализован сброс фотографий при отправке или обнулению формы.

- **`5.20`** Реализована загрузка фотографий в форму.
